...
assert(
  /aria-label="Fechar.*diagnostico"/i.test(modalComponent),
  'Close button must include the correct aria-label'
);
assert(
  modalComponent.includes('text-[var(--color-text-meta)]'),
  'Close button must use V2 meta text token'
);
assert(
  modalComponent.includes('hover:text-[var(--color-text-primary)]'),
  'Close button hover must use V2 primary text token'
);
